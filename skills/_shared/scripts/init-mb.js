...
function copyRuntimeScript(asset, target) {
  ...
  if (!absPath) {
    ...
    console.error('Run init-mb.js from the memobank package with mb-garden installed alongside mb-init.');
    process.exit(1);
  }
...
- \`.memory-bank/domains/runtime-data-model.md\`: Runtime data model.
- \`.memory-bank/contracts/api-guidelines.md\`: HTTP/API rules when HTTP boundary exists.
- \`.memory-bank/guides/frontend-component-guide.md\`: Frontend component/design behavior when UI component system is in scope.
...
description: Router for Memory Bank workflow policy documents.
status: active
owner: workflow
last_updated: ${TODAY}
source_of_truth:
...
---
# Workflow Policies Index

Use this router to find workflow policy documents before task execution,
verification, synchronization, or autonomous scheduling.

## Policies
- [mb-sync.md](mb-sync.md): Memory Bank synchronization checklist after meaningful changes.
- [tier-policy.md](tier-policy.md): TASK tier routing, protocol depth, and verification requirements.
- [autonomy-policy.md](autonomy-policy.md): Guardrails and terminal states for unattended runs.
- [execute-loop.md](execute-loop.md): PRD to feature to task execution loop.
`);
