...
function seedCommandsFromTemplates() {
  if (!fs.existsSync(COMMAND_TEMPLATES_DIR)) {
    console.error(`\nERROR: Command templates not found: ${COMMAND_TEMPLATES_DIR}`);
    console.error('Run init-mb.js from the memobank package (do not copy it standalone).');
    process.exit(1);
  }

  const files = listMarkdownFiles(COMMAND_TEMPLATES_DIR);
  const metas = [];

  files.forEach((filename) => {
    const abs = path.join(COMMAND_TEMPLATES_DIR, filename);
